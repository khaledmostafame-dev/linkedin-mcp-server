...
- `references["reactions"]` (get_post_reactions) — `kind: "person"` entries for each reactor profile, read from the post's reactions dialog. The dialog is opened and scrolled through a structural DOM probe (`scraping/reactions.py`), never a text match; when the probe cannot identify the social-counts control or the dialog unambiguously, `section_errors["reactions"]` carries `error_type: "structural_signal_not_found"` instead. Reaction type (Like/Celebrate/...) is not extracted.
- `save_post(post_url, confirm, unsave=False)` — a write tool. The overflow-menu opener and the save/unsave toggle inside it are both found structurally (`aria-expanded` presence, then the single `aria-pressed` control in the resulting menu — see `scraping/post_actions.py`), never by label text. `confirm=False` opens the menu (the only way to read current state) but never clicks the toggle. Both DOM probes here and in `get_post_reactions` are **unverified against a live account** — this fork never signs in to LinkedIn — and must be confirmed before being trusted.
