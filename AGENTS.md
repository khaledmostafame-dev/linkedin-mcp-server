...
- `total: {count: int, exact: bool}` (search_jobs only) — LinkedIn's own advertised result count read from the first page ("28 results", "1,000+ results" -> `exact: false`); absent when that line could not be parsed
...
- `references["feed_hashtag"]` (get_hashtag_feed) — same `feed_post`-style permalink capture as `references["feed"]` / `references["posts"]`; other reference kinds (mentioned people/companies) may also appear, matching `get_company_posts`. Unverified against a live account that the hashtag feed carries the same SDUI/GraphQL permalink payload as the home feed — see `feed_payload.is_post_listing_page`.
- `references["reactions"]` (get_post_reactions) — `kind: "person"` entries for each reactor profile, read from the post's reactions dialog. The dialog is opened by clicking the single `[data-reaction-details]` control in `<main>` and scrolled through a structural DOM probe (`scraping/reactions.py`), never a text match; when the probe cannot identify that control or the dialog unambiguously (including a dialog already on the page), `section_errors["reactions"]` carries `error_type: "structural_signal_not_found"` instead. Reaction type (Like/Celebrate/...) is not extracted.
- `save_post(post_url, confirm, unsave=False)` — a write tool. The overflow-menu opener and the save/unsave item inside it are both found structurally, never by label text: the opener is the one `button[aria-expanded]` above the post's first `[data-finite-scroll-hotkey]` action whose next sibling is its `aria-hidden` panel; the item is the one panel item holding a `bookmark-outline-*` (not saved) or `bookmark-fill-*` (saved) icon hook (see `scraping/post_actions.py`). `confirm=False` opens the menu (the only way to read current state) but never clicks the item; after a confirmed click the menu is reopened to read the new state.
- `delete_post` / `edit_post` open the same post menu through that opener rule (`post_actions.POST_MENU_JS`), never an overflow-icon match (on a live post only comment menus carry that icon); owner edit/delete items are looked up inside the opener's own panel, a panel holding a comment URN is refused, and edit_post takes its editor from the opened dialog, not the page's comment box. Both share one ownership check (`PostComposer._open_own_post_menu`): after a bounded wait for the post to render (menu opener and author links present; a post page first paints a boot loader, then `<main>`, then the post), the author is read only from the links preceding the opener inside the post unit and compared by vanity or profile id where both sides expose one (`post_composer.authorship`). `not_own_post` needs a positive contradiction; an unreadable or incomparable author is `author_unverified`, and a menu that never opens as the post's is `menu_unavailable`.
- Live status of these probes (signed-in English-UI post page, 2026-09-17 structural capture): the reactions control, the post menu opener, the comment box (a role=textbox editor beside an empty contenteditable helper), the top-level comment loaders and the per-comment Reply buttons match it. The reactions dialog, every opened menu's contents, the delete confirmation and the edit dialog were not in that capture and remain to be confirmed live.
